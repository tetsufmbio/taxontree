...
By default, the statistic support of a branch are evidenced by the size of the nodes, i.e. bigger the size, higher is the support. If you want to know their values go to *Node label* on FigTree side menu and choose *BOOT* on *Display*.

### Evidencing duplication nodes

TaxOnTree also annotates those nodes that represent duplication event. To evidence by node size, on FigTree side menu, go to *Node shapes* and choose *dup* on *Size by*. Set also the *Max size* and *Min size*.
